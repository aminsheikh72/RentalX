...
const CarDetails = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <div className="max-w-7xl mx-auto bg-white rounded-lg shadow-lg overflow-hidden mb-10">
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-6 mb-8">
              <div className="flex items-center gap-2">
                ...
                <span className="text-gray-600 uppercase">Category: {car.category}</span>
              </div>
              <div className="flex items-center gap-2">
                <Car className="w-5 h-5 text-gray-400" />
                <span className="text-gray-600 uppercase">Company: {car.company}</span>
              </div>
              <div className="flex items-center gap-2">
                <Fuel className="w-5 h-5 text-gray-400" />
                <span className="text-gray-600 uppercase">Fuel Type: {car.fuelType}</span>
              </div>
              <div className="flex items-center gap-2">
                <Car className="w-5 h-5 text-gray-400" />
                <span className="text-gray-600 uppercase">Registration: {car.registration}</span>
              </div>
            </div>
            <button
      ...
      {showBookingForm && (
        <div className="bg-white p-6 rounded-lg shadow-lg mt-6 mx-8 border">
          <h2 className="text-2xl font-semibold mb-4 text-gray-800">Booking Details</h2>
          <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-4">
            <div className="flex flex-col">
              <label className="text-gray-700 mb-2">Pickup Date</label>
              <input
                type="date"
                value={pickupDate}
            ...
            <div className="flex flex-col">
              <label className="text-gray-700 mb-2">Drop Date</label>
              <input
                type="date"
                value={dropDate}
      ...
      <div className="w-full mx-auto bg-white p-8 rounded-lg shadow-md mt-5">
        <h2 className="text-2xl font-bold text-gray-900 mb-4">Comments & Ratings</h2>
          ...
          {allComments.length > 0 ? (
            allComments.map((review) => (
              <div key={review._id} className="p-4 border border-gray-200 mb-2">
                ...
                <p className="text-gray-700 uppercase">user: {review.userName}</p>
                <p className="text-gray-700 uppercase">review: {review.comment}</p>
              </div>
            ))
          ) : (
